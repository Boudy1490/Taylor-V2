...
const connectionOptions = {
  ...
  patchMessageBeforeSending: message => {
    var requiresPatch = !!(message.buttonsMessage || message.templateMessage || message.listMessage);
    if (requiresPatch) {
      message = {
        viewOnceMessage: {
          message: {
  ...
  userDevicesCache: msgRetryCounterCache,
...
async function connectionUpdate(update) {
  ...
  if (connection === "connecting") {
    ...
    if (timeout > 30) {
      console.log("Taylor-V2".main, ">>".yellow, "Session logout after 30 times reconnecting. Saving your number from being banned!".warn);
...
const runTasks = async () => {
  ...
  try {
    const results = await Promise.allSettled(tasks.filter(t => t.run).map(async ({
      func,
      message
    }) => {
      try {
        spinner.start(chalk.bgYellow.black(`Running task: ${message}`));
        await func();
        spinner.succeed(chalk.bgYellow.black(`${message} completed`));
        return {
          status: "fulfilled",
          message: message
        };
      } catch (error) {
        spinner.fail(`Error in ${message}`);
  ...
  const importPromises = lodash.map(CommandsFiles, async file => {
    ...
    try {
      ...
      if (err) {
        ...
        const module = await (await import(file))?.default || await await import(file);
  ...
  try {
    if (existsSync(dir)) {
      ...
      if (err) {
        ...
        const module = await await import(`${Helper.__filename(dir)}?update=${Date.now()}`);
  ...
  try {
    ...
    const deletedFiles = await Promise.all(lodash.map(filenames, async file => {
      try {
        ...
        const stats = statSync(filePath);
        if (stats.isFile() && file.endsWith(".json") && file !== "creds.json") {
          unlinkSync(filePath);
          return filePath;
        }
      } catch (err) {
