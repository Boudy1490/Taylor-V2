...
const clockString = ms => {
  ...
  return `${tahun ? `*\`${tahun}\`* Tahun ` : ""}` + `${bulan || tahun ? `*\`${bulan}\`* Bulan ` : ""}` + `${minggu || bulan || tahun ? `*\`${minggu}\`* Minggu ` : ""}` + `${hari || minggu || bulan || tahun ? `*\`${hari}\`* Hari ` : ""}` + `${jam || hari || minggu || bulan || tahun ? `*\`${jam}\`* Jam ` : ""}` + `${menit || jam || hari || minggu || bulan || tahun ? `*\`${menit}\`* Menit ` : ""}` + `*\`${detik}\`* Detik`;
};
  ...
  return new Promise((resolve, reject) => {
    axios.get(`https://www.wallpaperflare.com/search?wallpaper=${query}`, {
      headers: {
        ...
        cookie: "_ga=GA1.2.863074474.1624987429; _gid=GA1.2.857771494.1624987429; __gads=ID=84d12a6ae82d0a63-2242b0820eca0058:T=1624987427:RT=1624987427:S=ALNI_MaJYaH0-_xRbokdDkQ0BNSlKaa0Hg; __gpi=UID=00000c80e037ba0f:T=1624987427:RT=1624987427:S=ALNI_MaJkE-55hGUlNTe87I5YhoD1Q-0zA; _gat=1"
      }
    }).then(({
      data
    }) => {
      ...
      const hasil = [];
      $("div.item").each((l, h) => {
        hasil.push($(h).find("img").attr("src").replace(/small|_small/g, "large"));
      });
      resolve(hasil);
    }).catch(reject);
  });
...
const playstore = async name => {
  try {
    const response = await fetch(`https://play.google.com/store/search?q=${encodeURIComponent(name)}&c=apps`);
    ...
    const dv = [];
    const lm = [];
    $("div.wXUyZd > a").each((_, elem) => ln.push("https://play.google.com" + $(elem).attr("href")));
    $("div.b8cIId.ReQCgd.Q9MA7b > a > div").each((_, elem) => nm.push($(elem).text().trim()));
    $("div.b8cIId.ReQCgd.KoLSrc > a > div").each((_, elem) => dv.push($(elem).text().trim()));
    $("div.b8cIId.ReQCgd.KoLSrc > a").each((_, elem) => lm.push("https://play.google.com" + $(elem).attr("href")));
...
const linkwa = async nama => {
  try {
    const response = await fetch(`http://ngarang.com/link-grup-wa/daftar-link-grup-wa.php?search=${encodeURIComponent(nama)}&searchby=name`);
    const data = await response.text();
    const $ = cheerio.load(data);
    const result = [];
    const lnk = [];
    const nm = [];
    $("div.wa-chat-title-container a").each((_, elem) => lnk.push($(elem).attr("href")));
...
const generate = n => {
  ...
  return ("" + (Math.floor(Math.random() * (Math.pow(10, n + 1) - min + 1)) + min)).substring(1);
};
const delay = time => new Promise(resolve => setTimeout(resolve, time));
const isUrl = text => /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)(jpe?g|gif|png)/gi.test(text);
...
const GDriveDl = async url => {
  if (!url || !url.match(/drive\.google/i)) return {
    error: true
  };
  try {
    ...
    const {
      ...
      sizeBytes,
      downloadUrl
    } = JSON.parse((await response.text()).slice(4));
...
const summarize = async (input, num = 50) => {
  try {
    const response = await fetch(`https://api.text-summarize.com/summarize?ratio=${num}`, {
      method: "POST",
      headers: {
...
const ViewSource = async url => {
  try {
    const response = await fetch("https://viewsourcepage.com/wp-admin/admin-ajax.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
        Accept: "*/*",
