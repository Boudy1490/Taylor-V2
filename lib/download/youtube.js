...
const Ytdl = {
  ...
  all: async (u, f) => {
    ...
    try {
      ...
      return headRes.ok && (contentType?.startsWith("video") || contentType?.startsWith("audio")) ? {
        url: downloadUrl,
        buffer: Buffer.from(await fetch(downloadUrl).then(res => res.arrayBuffer())),
        contentType: contentType,
        title: new Date().toISOString()
      } : null;
    } catch {
...
async function getMP3(videolink, frmt = "mp3") {
  return new Promise(async (resolve, reject) => {
    try {
      if (-1 < videolink.indexOf("youtu.be") || -1 < videolink.indexOf("youtube.com/shorts/")) {
        videolink = "https://www.youtube.com/watch?v=" + /\/([a-zA-Z0-9\-\_]{11})/.exec(videolink)[1];
      } else if (-1 < videolink.indexOf("youtube.com")) {
        videolink = "https://www.youtube.com/watch?v=" + /v\=([a-zA-Z0-9\-\_]{11})/.exec(videolink)[1];
      } else {
        videolink = null;
      }
      ...
      const mainresp = await progress(`${conlink.convertURL}&v=${videolink}&f=${frmt}&_=${Math.random()}`);
      await checkProgress(mainresp.progressURL);
      resolve(mainresp.downloadURL);
      async function validate(response) {
        return new Promise((resolve, reject) => {
          if (response.status != 200) return reject(response.statusText || "NO STATUS");
          const conlink = response.data;
          if (conlink.error != "0") return reject(JSON.stringify(conlink) || "NO ERROR");
