        ...
        try {
          ...
          switch (true) {
            ...
            case /^image\//.test(mime):
              ...
              file = buffer || file;
              mtype = buffer ? "sticker" : /^image\/gif/.test(mime) ? "document" : "image";
              mimetype = buffer ? "image/webp" : options.mimetype || (/^image\/gif/.test(mime) ? "document" : mime);
              break;
            case /^video\//.test(mime):
              bufferAudio = options.as8k ? await toAudio8k(file, ext) : options.asAudio ? await toAudio(file, ext) : null;
              if (bufferAudio) {
            ...
            case /^audio\//.test(mime):
              buffer = options.as8k ? await toAudio8k(file, ext) : await toAudio(file, ext);
          ...
          const message = {
            ...mergeContext(conn, caption, options),
            caption: caption || "",
            ptt: ptt,
            [mtype]: pathFile ? {
              url: pathFile
            ...
            fileName: (filename?.includes("/") ? filename.split("/").pop() : filename) || (pathFile?.includes("/") ? pathFile.split("/").pop() : pathFile) || "audio.mp3"
          };
          try {
            if (options.asPtv) {
              return await conn.relayMessage(jid, await generateWAMessageFromContent(jid, proto.Message.fromObject({
                [mtype]: pathFile ? {
                  url: pathFile
                } : base64File
              }), {
...
END:VCARD`.trim();
    ...
    sendSlide: {
      async value(jid, text = "", footer = "", slides = [], quoted, options = {}) {
        ...
        for (const [txt, ftr, header, thumbnailUrl, buttons] of slides.slice(0, 7)) {
          ...
          if (!isAndroid) {
            return await conn.sendBtn(jid, txt, footer, thumbnailUrl, buttons, quoted);
          }
          try {
    ...
    ctaCButton: {
      get() {
        class Button {
          ...
          async run(jid, conn, quoted = {}) {
            const cards = await Promise.all(this._cards.map(async card => {
              const media = card.media ? await prepareWAMessageMedia(card.media, {
                upload: conn.waUploadToServer
              }) : {};
              return {
                header: proto.Message.InteractiveMessage.Header.create({
    ...
    send2ButtonImgDoc: {
      async value(jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) {
        ...
        await conn.sendMsg(jid, buttonMessage, {
          quoted: quoted,
          ephemeralExpiration: ephemeral,
          contextInfo: {
    ...
    send1Button: {
      async value(jid, content, footerText, button1, id1, quoted, options) {
        ...
        conn.sendMsg(jid, buttonMessage, {
          quoted: quoted,
          ephemeralExpiration: ephemeral,
          contextInfo: {
    ...
    reply: {
      async value(jid, text = "", quoted, options = {}) {
        try {
          ...
          let m = null;
          try {
            m = data ? await conn.sendFile(jid, data, "file", "", quoted, false, options) : await conn.sendMsg(jid, {
              text: text,
              ...mergeContext(conn, text, options)
            }, {
              quoted: quoted,
    ...
    sendHydrated2: {
      async value(jid, text = "", footer = "", buffer, url, urlText, url2, urlText2, buttons, quoted, options) {
        ...
        await conn.sendMsg(jid, message, {
          quoted: quoted,
          upload: conn.waUploadToServer,
          ...options
        });
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    isBaileys: {
      get() {
        return _.some(["BAE", "B1E", "3EB0", "WA"], pfx => _.startsWith(this.id, pfx)) || _.includes([12, 16, 20, 22, 40], this.id?.length) || _.includes(["BAE", "B1E", "3EB0", "WA"], this.id);
      }
    },
    chat: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === "string" ? {
          ...
          upload: {
            async value(provider = "all", saveToFile = false) {
              const mtype = this.mediaType;
              return await uploadFile(await self.conn?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ""), saveToFile), provider);
            },
            enumerable: true,
            configurable: true
          },
          ...
          delete: {
            value(key = null) {
              return self.conn?.sendMsg(this.chat, {
    ...
    upload: {
      async value(provider = "all", saveToFile = false) {
        const mtype = this.mediaType;
        return await uploadFile(await this.conn?.downloadM(this.mediaMessage[mtype], mtype.replace(/message/i, ""), saveToFile), provider);
      },
      enumerable: true,
      configurable: true
    },
    ...
    delete: {
      value(key = null) {
        return this.conn?.sendMsg(this.chat, {
          delete: key || this.key
        });
      },
      enumerable: true
...
function mergeContext(conn, text, options) {
  return _.merge({
    ...
    contextInfo: _.merge(options?.contextInfo || {}, db.data.dbbot.temareply.contextInfo, {
      externalAdReply: _.merge(options?.contextInfo?.externalAdReply || {}, db.data.dbbot.temareply.contextInfo.externalAdReply)
    })
  } : {});
}