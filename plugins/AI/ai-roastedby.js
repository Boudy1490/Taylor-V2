      ...
      case "roastedbyset":
        if (text) {
          ...
          if (!isNaN(index) && index >= 1 && index <= styles.length) {
            ...
            const buttons = conn.ctaButton.setBody("Select Style Below.").setFooter("More details can be found at RoastedBy.").addSelection("Click Here", `${usedPrefix}roastedbyset`).makeSections("RoastedBy", "Select Style");
            styles.forEach((style, idx) => {
              buttons.makeRow("", style, `Style: ${style}`, `${usedPrefix}roastedbyset ${idx + 1}`);
            });
            return buttons.run(m.chat, conn, m);
          }
        } else {
          m.reply(`Please provide an index to set the style. Example: ${usedPrefix}roastedbyset 1`);
        }
        break;
    }
  } catch (error) {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.roastedby || m.isBaileys || !(m.sender in db.data.dbai.roastedby)) return;
  const {
    style
  } = db.data.dbai.roastedby[m.sender];
  if (m.quoted?.id === db.data.dbai.roastedby[m.sender]?.keyId && style && m.text.trim()) {
    m.react(wait);
    try {
