  ...
  try {
    switch (command) {
      case "caishann":
        ...
        const sessionName = db.data.dbai.caishann[m.sender]?.sessionName || uuidv4();
        if (characterId) {
          const {
            answer
          } = await Caishann(text, characterId, sessionName);
          if (answer) {
          ...
          m.reply("No character ID set. Use .caishannset command to set a character ID. ❗");
        }
        break;
      case "caishannset":
        if (text) {
          const index = parseInt(text.trim(), 10);
          if (!isNaN(index) && index >= 1 && index <= Object.keys(characters).length) {
            const characterId = Object.keys(characters)[index - 1];
            db.data.dbai.caishann[m.sender] = {
              characterId: characterId,
              sessionName: uuidv4()
            };
            m.reply(`Character ID set to ${characters[characterId]} successfully! ✅`);
          } else {
            const buttons = conn.ctaButton.setBody("Select Character Below.").setFooter("More details can be found at CaiShann.").addSelection("Click Here", `${usedPrefix}caishannset`).makeSections("CaiShann", "Select Character");
            Object.keys(characters).forEach((key, index) => {
              buttons.makeRow(``, `${characters[key]}`, `Character ID: ${key}`, `${usedPrefix}caishannset ${index + 1}`);
            });
            return buttons.run(m.chat, conn, m);
          }
        } else {
          m.reply(`Please provide an index to set the character ID. Example: ${usedPrefix}caishannset 1`);
        }
        break;
    }
  } catch (error) {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.caishann || m.isBaileys || !(m.sender in db.data.dbai.caishann)) return;
  const {
    characterId,
    sessionName
  } = db.data.dbai.caishann[m.sender];
  if (m.quoted?.id === db.data.dbai.caishann[m.sender]?.keyId && characterId && m.text.trim()) {
    m.react(wait);
    try {
      const {
        answer
      } = await Caishann(m.text.trim(), characterId, sessionName);
      if (answer) {
