    ...
    switch (command) {
      case "crushchat":
        ...
        if (personaId) {
          const {
            reply
          } = await CrushChat(text, personaId);
          if (reply) {
            const {
              key: {
                id: keyId
          ...
          m.reply("No persona ID set. Use .crushchatset or .crushchatsearch command to set persona ID. ❗");
        }
        break;
      case "crushchatsearch":
        const searchResults = await SearchPersona(text);
        if (searchResults.length) {
          const buttons = conn.ctaButton.setBody("Select Character Below.").setFooter("More details can be found at CrushChat.").addSelection("Click Here", `${usedPrefix}crushchatsearch ${text}`).makeSections("CrushChat", "Select Character");
          searchResults.forEach((result, index) => {
            buttons.makeRow(``, `${result.name}`, `${result.description}`, `${usedPrefix}crushchatset ${result.persona}`);
          });
      ...
      case "crushchatset":
        if (text) {
          ...
          m.reply(`Please provide a persona ID to set. Example: ${usedPrefix}crushchatset your_persona_id`);
        }
    }
  } catch (error) {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.crushchat || m.isBaileys || !(m.chat in db.data.dbai.crushchat)) return;
  const {
    keyId,
    personaId
  } = db.data.dbai.crushchat[m.chat];
  if (m.quoted?.id === keyId && personaId && m.text.trim()) {
    ...
    try {
      const {
        reply
      } = await CrushChat(m.text.trim(), personaId);
      if (reply) {
        const message = reply;
        const {
          key: {
